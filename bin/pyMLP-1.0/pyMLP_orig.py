...
class Defaults(object):
    ...
    def __init__(self):
        ...
        self.fidatadefault = {  # Default fi table
            "ALA": {
                "CB": "0.63",  # fi : lipophilic atomic potential
                "C": "-0.54",
                "CA": "0.02",
                "O": "-0.68",
                "N": "-0.44",
...
def _CLIparsing():
    ...
    CLparser.add_option(
        ...
        type="float",
        dest="spacing",
        help="Grid spacing (default = 1.0 Angstrom)",
        metavar="1.0",
    )
    CLparser.add_option(
        "-n",
...
def writefitab(fidata, fitabfile, verbose=False):
    ...
    try:
        ...
        fitabf.write("fidata = {\n " + pformat(fidata)[1:])
        if verbose:
            sys.stdout.write("%s created with default values...\n" % fitabfile)
    except IOError:
...
class Molecule(object):
    ...
    def writefipdb(self, fipdbfile):
        ...
        if self.verbose:
            sys.stdout.write("%s was writen" % fipdbfile)
    ...
    def _griddimcalc(listcoord, spacing, gridmargin):
        ...
        adjustment = ((spacing - (coordmax - coordmin)) % spacing) / 2.0
