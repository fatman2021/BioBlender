...
def computeNormalMode(
    userfilenamein="",
    userfilenameout="NMA.pdb",
    usermode=0,
    userrmsd=0.8,
    usernbconf=5,
    conf="allatom",
    ...
    ensemble = traverseMode(
        bb_anm[usermode], bb_atoms, n_steps=usernbconf, rmsd=userrmsd
    )
    ...
    # Prody checks if coordsets> 1; should be >= 1...
    nmastruct.addCoordset(ensemble)
...
def main():
    ...
    if pdbin == "" or pdbout == "" or mode == 0:
        ...
        computeNormalMode(
            pdbin, pdbout, mode, rmsd, nbconformations, conf, cutoff, gamma
        )
